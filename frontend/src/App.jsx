...
function App() {
  ...
  const [requireAddressCheck, setRequireAddressCheck] = useState(() => {
    try {
      const saved = localStorage.getItem('requireAddressCheck');
      return saved === 'true'; // defaults to false (off) if never set
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('requireAddressCheck', String(requireAddressCheck));
    } catch {
  ...
  // or if the requirement is toggled — re-enabling it shouldn't grandfather
  // in a confirmation that happened while it was off.
  useEffect(() => {
    setAddressConfirmed(false);
    setAddressSuggestion(null);
    setAddressCheckError('');
  }, [clientStreetAddress, clientCity, clientState, clientZip, requireAddressCheck]);
  ...
  const handleCalculate = (e) => {
    ...
    if (isInvoice && (!requiredInvoiceFieldsFilled || !verificationVerified || (requireAddressCheck && !addressConfirmed))) {
      setCalculatedValue(null);
      setGrossValue(null);
      setFeeAmount(null);
      return;
              ...
              <form onSubmit={handleCalculate} style={{ display: 'flex', flexDirection: 'column', gap: '14px', textAlign: 'left', fontFamily: 'sans-serif' }}>
                ...
                {isInvoice && (
                  <>
                    <div>
                      <label style={{ fontSize: '11px', color: '#94a3b8', display: 'block', marginBottom: '5px', letterSpacing: '1px', fontWeight: '600' }}>
                        REQUIRE USPS ADDRESS CONFIRMATION?
                      </label>
                      <select
                        value={requireAddressCheck ? 'yes' : 'no'}
                        onChange={(e) => setRequireAddressCheck(e.target.value === 'yes')}
                        style={{ width: '100%', padding: '12px', background: '#090d16', border: '1px solid rgba(212, 175, 55, 0.2)', borderRadius: '8px', color: '#fff', fontSize: '14px', outline: 'none' }}
                      >
                        <option value="no">No</option>
                        <option value="yes">Yes</option>
                ...
                <button
                  type="submit"
                  disabled={isInvoice && (!requiredInvoiceFieldsFilled || !verificationVerified || (requireAddressCheck && !addressConfirmed))}
                  style={{
                    width: '100%',
                    padding: '14px',
                    background: isInvoice && (!requiredInvoiceFieldsFilled || !verificationVerified || (requireAddressCheck && !addressConfirmed))
                    ...
                    fontWeight: 'bold',
                    fontSize: '13px',
                    cursor: isInvoice && (!requiredInvoiceFieldsFilled || !verificationVerified || (requireAddressCheck && !addressConfirmed)) ? 'not-allowed' : 'pointer',
                    letterSpacing: '1px',
