...
function App() {
  ...
  const [customFee, setCustomFee] = useState('10.00');
  ...
  const handleCalculate = (e) => {
    ...
    if (!metals || !weight || Number.isNaN(Number(weight)) || Number(weight) <= 0) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginBottom: '25px', width: '100%' }}>
            <div style={{ backgroundColor: 'rgba(15, 23, 42, 0.7)', borderRadius: '16px', padding: '25px', border: '1px solid rgba(212, 175, 55, 0.25)', boxShadow: '0 20px 40px rgba(0,0,0,0.5)', backdropFilter: 'blur(10px)' }}>
              ...
              <form onSubmit={handleCalculate} style={{ display: 'flex', flexDirection: 'column', gap: '14px', textAlign: 'left', fontFamily: 'sans-serif' }}>
                ...
                <button
                  type="submit"
                  style={{ width: '100%', padding: '14px', background: 'linear-gradient(135deg, #d4af37 0%, #aa841c 100%)', border: 'none', borderRadius: '8px', color: '#000000', fontWeight: 'bold', fontSize: '13px', cursor: 'pointer', letterSpacing: '1px', textTransform: 'uppercase', boxShadow: '0 4px 20px rgba(212, 175, 55, 0.2)' }}
                >
